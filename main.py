...
@app.route('/')
def home():
    return render_template("index.html", logeed_in = current_user.is_authenticated)
...
def login():
    if request.method == "POST":
        ...
        if not user:
            flash("That email id does not exist, Please try again")
            return redirect(url_for('login'))
...
@login_required
def download():
    # path='static/files/cheet_sheet.pdf'
